...
# tests 1-20 pencil, tests 21-40 tennis_ball
...
with (OUT / "selected_images.txt").open("w", encoding="utf-8") as f:
    ...
    f.write(f"selection_rule=20 pencil + 20 tennis_ball, single-GT-object test images only\n")
...
summary = {
    ...
    "meets_40_tests": total == 40,
    "meets_accuracy_80_percent": (
        total == 40 and correct_n / total >= 0.80
    ),

    "notes": [
        "40 cases were selected only from dataset/images/test.",
        "dataset/images/val was not used.",
        "Each selected image contains exactly one ground-truth object.",
        "Tests 1-20 are pencil and tests 21-40 are tennis_ball.",
...
with (OUT / "test_plan.md").open(
    ...
    f.write(
        "# 40-Case Test-Set Acceptance Plan\n\n"
        f"- Random seed: `{SEED}`\n"
        f"- Dataset split: `test`\n"
        f"- Validation split used: `No`\n"
        f"- Pencil cases: `20`\n"
        f"- Tennis-ball cases: `20`\n"
