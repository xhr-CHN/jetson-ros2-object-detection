...
图\ref{fig:training}为完整100轮训练记录的黑白排版副本，原始彩色图保存在结果目录。训练损失整体下降，验证指标在后期趋于稳定。最佳权重出现在第87轮，而不是人为中断得到的临时权重；本项目最终模型来自完整跑完100轮的正式增强训练。
...
图\ref{fig:pr}给出了独立测试的PR曲线。两类曲线均靠近左上区域，说明在较宽的置信度阈值范围内具有较好的Precision--Recall平衡。该图由Ultralytics标准评估流程生成，报告中仅转换为灰度，未修改数据。
